...
def main():
    ...
    # ─── 2. 토글 버튼 (사이드바 안쪽 상단에 배치) ───
    #    버튼 클릭 시 sidebar_hidden 값을 뒤집습니다.
    with st.sidebar:
        clicked = st.button(
            "◀" if not st.session_state.sidebar_hidden else "▶",
    ...
    <style>
      ...
      /* 3) 토글 버튼 스타일 (사이드바 내부 상단) */
      [data-testid="stSidebar"] .stButton > button#toggle_sidebar_btn {{
        width: 32px;
        height: 32px;
        padding: 0;
        margin-bottom: 16px;
      ...
      [data-testid="stSidebar"] .stButton > button#toggle_sidebar_btn:hover {{
    ...
    with st.sidebar:
        if st.session_state.sidebar_hidden:
            return  # 사이드바 완전 닫혔을 때는 바로 리턴해서 내부 요소 노출 방지
