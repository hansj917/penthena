...
def display_core_offers(text: str):
    """‘핵심 오퍼’ 마크다운 테이블을 받아서 3개의 둥근 박스(UI 카드)로 렌더링"""
    df = parse_table_from_text(text)
    if df.empty:
        st.warning("핵심 오퍼 데이터를 분석할 수 없습니다.")
        return
        ...
        with cols[i % min(len(df), 3)]:
            st.markdown(f"""
                <div class="offer-box">
                  <h4>{row['오퍼 내용']}</h4>
                  <p>{row['조건']}</p>
                  <p class="offer-score">매력도: {row['매력도(10점)']}/10</p>
                </div>
            """, unsafe_allow_html=True)
...
def promotion_planning_pipeline(topic: str, research_context: str):
    ...
    steps = {
        ...
        "핵심 오퍼": {"prompt_template": "경쟁사와 트렌드, 우리 제품 기능을 고려하여 '{p}' 프로모션의 매력적인 혜택 3가지를 '오퍼 내용', '조건', '매력도(10점)' 컬럼의 마크다운 테이블로 만들어줘.", "display_type": "custom", "func": display_core_offers},
